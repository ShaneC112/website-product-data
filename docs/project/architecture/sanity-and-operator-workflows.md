...
When product-level native `aiTemplates` are present, Azure also derives one colour-neutral texture prompt per product/source-fingerprint/prompt-version for non-`none` requests. The swatch and hex colour remain authoritative for floor colour, while the template roomsets steer installed surface character such as pile roughness, sheen, tonal breakup, and room-scale material behavior. The resolved texture prompt is cached on the product, and each prepared run snapshots the exact texture prompt/fingerprint pair it used before any later generate step. Products without templates keep the legacy preparation path.
