...
| Project Pipeline Developer | Implements bounded approved Data, Azure, and Render durability slices with Data-first contracts, npm, focused tests, and parent-owned integration. It cannot delegate or perform protected live, dependency, Azure configuration, Sanity mutation, or Git operations without exact fresh approval. |
| Project Pipeline Reviewer | Performs strictly read-only review of queue, journal, ledger, claim, outbox, retry, recovery, fan-out, lifecycle, cleanup, status, provider-side-effect, and missing-handoff correctness. It cannot edit, execute, delegate, or touch live state. |
