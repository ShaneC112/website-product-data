...
9. Identify every phase affecting queue contracts, Table or blob journals, stage ledgers, outboxes or dispatch intents, claims or leases, retries or recovery, fan-out completion, lifecycle state, cleanup or finalization, status projection, provider side effects, or missing handoffs. Obtain pre-change state-machine and interruption-boundary findings from the read-only `project-pipeline-reviewer`.
...
Prefer `project-pipeline-developer` over a generic Project Implementor worker or Project Engineer for bounded Data, Azure, or Render durability implementation. Give it an approved file manifest, state-transition invariants, and focused validation; keep shared Data contracts and their build ahead of Azure or Render consumers. The specialist cannot delegate, change dependencies, deploy, enqueue or drain queues, clear or mutate shared storage, run write-capable live E2E, reconfigure Azure, write Sanity, commit, or push without exact fresh approval. The parent retains diff inspection, integration, broader validation, and acceptance.
...
10. For a durable pipeline phase, invoke `project-pipeline-reviewer` on the settled diff before acceptance. Independently reconcile its lost-work, duplicate-side-effect, generation-fencing, claim recovery, missing-handoff, cleanup, and status-evidence findings; the reviewer remains read-only and the parent owns any fixes.
11. Confirm every acceptance criterion with evidence before marking the phase complete or starting a dependent phase.
