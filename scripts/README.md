...
| `check-image-generation-v2-isolation.mjs` | `npm run image-generation:v2:isolation:check` | Emit the current v2 image-generation boundary report across Data, Azure, and Studio by copying a disposable OS temporary workspace, removing declared legacy registrations, and running scoped validation commands there. | **local-read with disposable temp-workspace write**; removes the temporary workspace before exit. | Exit status 0 and review the emitted JSON for known blockers. |
| `image-generation-v2-isolation.manifest.mjs` | none (helper manifest) | Declare the current v2 entrypoints, excluded roots, known blockers, and scoped validation commands used by the isolation checker. | **local-read** | Covered by `npm run image-generation:v2:isolation:check` and `npm run scripts:index:check` |
