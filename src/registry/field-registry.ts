...
export const fieldRegistry: FieldRegistryEntry[] = [
  ...
  createEntry('Carpet', 'width', 'Available roll width(s) when explicitly stated, as one measurement entry per distinct width.', 'recommended', 'measurement-list', { exampleValue: '[{"value":400,"unit":"cm"}]', promptHint: 'Return one array entry per distinct available roll width, each as {"value": number, "unit": string}. Prefer standalone roll width statements over full dimension sets. Common units: cm, m. May be labelled Width in cm or Available widths.' }),
