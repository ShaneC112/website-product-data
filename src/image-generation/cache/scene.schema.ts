...
export const sceneArtifactSchema = z.object({
  ...
  composition: z.object({shell: z.string().trim().min(1), furnishings: z.string().trim().min(1), paletteStrategy: z.string().trim().min(1), lighting: z.string().trim().min(1), stylingNotes: z.string().trim().min(1)}).strict(),
