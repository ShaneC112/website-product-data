...
/** Browser-safe synchronous SHA-256 used where stable request identity must match server evaluation. */
export function sha256(value: string): string {
  ...
  for (let offset = 0; offset < bytes.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 = (h + s1 + choice + ROUND_CONSTANTS[index]! + words[index]!) >>> 0
